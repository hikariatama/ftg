...
Version: {version}"""
    ...
    @error_handler
    @chat_command
    async def fnotescmd(self, message: Message) -> None:
            ...
            if fed in self.federations:
                if user_id and isinstance(message, Message):
                    if str(user_id) not in self.ratelimit['notes'] or \
                        ...
                        if not (message.raw_text.startswith(prefix) and len(message.raw_text) > 1 and \
                                message.raw_text[1] != prefix):
                            await self.update_feds()
                            if message.raw_text == "#заметки" or message.raw_text == "#notes":
                                self.ratelimit['notes'][str(user_id)] = time.time() + 3
                                await self.fnotescmd(await message.reply(f'<code>{prefix}fnotes</code>'))
