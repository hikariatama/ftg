...
class WelcomeMod(loader.Module):
	...
	async def watcher(self, message):
		...
		if getattr(message, "user_joined", False) or getattr(message, "user_added", False):
			...
			await self.client.send_message(cid, self.welcome[cid].replace('{user}', user.first_name).replace('{chat}', chat.title).replace('{mention}', '<a href="tg://user?id=' + str(user.id) + '">' + user.first_name + '</a>'), reply_to=message.action_message.id)
